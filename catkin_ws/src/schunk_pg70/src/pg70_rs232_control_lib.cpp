...
void PG70_serial::reference(serial::Serial *port)
{
...
uint8_t PG70_serial::getError(serial::Serial *port)
{
...
void PG70_serial::acknowledgeError(serial::Serial *port)
{
	std::vector<uint8_t> output;
	output.push_back(0x05);          //message from master to module
	output.push_back(gripper_id_);   //module id
...
void PG70_serial::setPosition(serial::Serial *port, int goal_position, int velocity, int acceleration)
{
...
void PG70_serial::setPVAC(serial::Serial *port, int goal_position, int velocity, int acceleration, int current)
{
	ROS_INFO_STREAM("PG70: Moving from: " << act_position_ << " [mm] to " << goal_position << " [mm]");
...
void PG70_serial::movePosTime(serial::Serial *port, int goal_position, int velocity, int acceleration, int current, int time)
{
	ROS_INFO_STREAM("PG70: Moving from: " << act_position_ << " [mm] to " << goal_position << " [mm]");
	ROS_INFO("PG70: Data used: %d [mm/s], %d [mm/s2], %d [A]", velocity, acceleration, current);
...
void PG70_serial::movePosTimeLoop(serial::Serial *port, int goal_position, int velocity, int acceleration, int current, int time)
{
	ROS_INFO_STREAM("PG70: Moving from: " << act_position_ << " [mm] to " << goal_position << " [mm]");
	ROS_INFO("PG70: Data used: %d [mm/s], %d [mm/s2], %d [A]", velocity, acceleration, current);
...
float PG70_serial::getPosition(serial::Serial *port)
{
	std::vector<uint8_t> output;
	output.push_back(0x05);                 //message from master to module
	output.push_back(gripper_id_);           //module id
...
float PG70_serial::getCurrent(serial::Serial *port)
{
	std::vector<uint8_t> output;
	output.push_back(0x05);                 //message from master to module
	output.push_back(gripper_id_);           //module id
...
void PG70_serial::getPeriodicPositionUpdate(serial::Serial *port, float update_period)
{
	std::vector<uint8_t> output;
...
void PG70_serial::stop(serial::Serial *port)
{
	std::vector<uint8_t> output;
	output.push_back(0x05);         //message from master to module
	output.push_back(gripper_id_);  //module id
...
bool PG70_serial::referenceCallback(schunk_pg70::reference::Request &req,
...
bool PG70_serial::setPositionCallback(schunk_pg70::set_position::Request &req,
                                 schunk_pg70::set_position::Response &res)
{
	
	int target_pos = req.goal_position * 1000;
	int target_vel = req.goal_velocity * 1000;
	int target_acc = req.goal_acceleration * 1000;

	
	if(checkRequestLimit(target_pos, target_vel, target_acc))
	{
		setPosition(com_port_, target_pos, target_vel, target_acc);
		res.goal_accepted = true;	
	}
	else
	{
		ROS_WARN("PG70: Goal not accepted");
...
bool PG70_serial::setPVACCallback(schunk_pg70::set_pvac::Request &req,
                                 schunk_pg70::set_pvac::Response &res)
{
	...
	if(checkRequestLimit(target_pos, target_vel, target_acc, req.goal_current))
	{
		setPVAC(com_port_,target_pos, target_vel, target_acc, req.goal_current);
		res.goal_accepted = true;
	}
	else
	{
...
bool PG70_serial::movePosTimeCallback(schunk_pg70::move_time::Request &req,
                                 schunk_pg70::move_time::Response &res)
{
	...
	if(checkRequestLimit(target_pos, target_vel, target_acc, req.goal_current))
	{
		movePosTime(com_port_, target_pos, target_vel, target_acc, req.goal_current, req.goal_time);
		res.goal_accepted = true;
	}
	else
	{
...
bool PG70_serial::movePosTimeLoopCallback(schunk_pg70::move_time_loop::Request &req,
                                 schunk_pg70::move_time_loop::Response &res)
{
	...
	if(checkRequestLimit(target_pos, target_vel, target_acc, req.goal_current))
	{
		movePosTimeLoop(com_port_, target_pos, target_vel, target_acc, req.goal_current, req.goal_time);
		res.goal_accepted = true;
	}
	else
	{
...
bool PG70_serial::getPositionCallback(schunk_pg70::get_position::Request &req,
...
bool PG70_serial::getCurrentCallback(schunk_pg70::get_current::Request &req,
...
bool PG70_serial::getErrorCallback(schunk_pg70::get_error::Request &req,
...
bool PG70_serial::acknowledgeErrorCallback(schunk_pg70::acknowledge_error::Request &req,
...
bool PG70_serial::stopCallback(schunk_pg70::stop::Request &req,
...
void PG70_serial::timerCallback(const ros::TimerEvent &event)
{
...
/*
float PG70_serial::IEEE_754_to_float(uint8_t *raw)
{
...
void PG70_serial::float_to_IEEE_754(float position, unsigned int *output_array)
{
...
uint16_t PG70_serial::CRC16(uint16_t crc, uint16_t data)
{
